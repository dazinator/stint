...
- Jobs:
  ...
  - Are classes with async methods with are run with cancellation tokens so you can exit gracefully, if for example the host is shutting down, or the job needs to be terminated for a config reload.
